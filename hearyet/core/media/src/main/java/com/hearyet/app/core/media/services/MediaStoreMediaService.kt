...
import com.hearyet.app.core.common.extensions.VIDEO_COLLECTION_URI
import com.hearyet.app.core.common.extensions.prettyName
...
class MediaStoreMediaService @Inject constructor(
    ...
    override suspend fun fetchVideos(folderPath: String?): List<MediaVideo> = withContext(Dispatchers.IO) {
        return@withContext runMediaStoreQuery {
            ...
            val (selection, selectionArgs) = videoSelection(folderPath)
    ...
    override suspend fun countVideos(folderPath: String?): Int = withContext(Dispatchers.IO) {
        val (selection, selectionArgs) = videoSelection(folderPath)
    ...
    override suspend fun fetchVideosAfter(afterId: Long, limit: Int, folderPath: String?): List<MediaVideo> =
        withContext(Dispatchers.IO) {
            return@withContext runMediaStoreQuery {
                val mediaVideos = mutableListOf<MediaVideo>()
                val (selection, selectionArgs) = videoSelection(folderPath, afterId)
                // Ordered by _ID so the next page can continue from the last seen id. A LIMIT
                // clause in the sort order is rejected by some OEM MediaStore providers
                // ("Invalid token LIMIT"), so the cursor is simply read for at most [limit]
                // rows and the provider stops delivering the rest.
                ...
                context.contentResolver.query(
                    ...
                    while (mediaVideos.size < limit && cursor.moveToNext()) {
                        val video = cursor.toMediaVideo() ?: continue
                        mediaVideos.add(video)
                    }
                }
    ...
    /**
     * Builds the selection for the given folder path, or null to scan every storage volume.
     * For a specific folder, matches it and its descendants, escaping LIKE metacharacters
     * ('%', '_') in the path. [afterId] restricts the selection to videos with a larger id.
     */
    private fun videoSelection(folderPath: String?, afterId: Long = 0L): Pair<String?, Array<String>?> {
        val pathSelection = if (folderPath == null) {
            null
        } else {
            "${MediaStore.Video.Media.DATA} LIKE ? ESCAPE '\\'"
        }
        val pathArgs = if (folderPath == null) {
            null
        } else {
            arrayOf("${folderPath.escapeLike()}/%")
        }
        return when {
            afterId <= 0L -> pathSelection to pathArgs
            pathSelection == null -> "${MediaStore.Video.Media._ID} > ?" to arrayOf(afterId.toString())
            else -> "$pathSelection AND ${MediaStore.Video.Media._ID} > ?" to (pathArgs!! + arrayOf(afterId.toString()))
        }
    }
